...
2. From the Services search box click on VPC and then from the left tab click on "Your VPCs" and finally click the upper button "Create VPC"
	1. Name tag: **DSTI_A19_Project1**
	1. IPv4 CIDR block: **14.80.0.0/20**, which means this VPC includes 4,096 IPs between 14.80.0.0 and 14.80.15.255
	1. We click on Close
	
### Step 2: Create the Public Subnet
1. Click on "Subnets" from the left tab and then on the button "Create Subnet"
	1. Name tag: **PublicSubnet1_A19_P1**
	1. VPC: we select the ID for the DSTI_A19_Project1 VPC
	1. IPv4 CIDR block: **14.80.0.0/24** 256 IPs (5 of which are reserved and unusable) IP range between 14.80.0.0 and 14.80.0.255
...
1. From the left tab list click on Subnets, click on PublicSubnet1_A19_P1, from down click on the tab "Route Table" and then click on the  link of the Route table ID"
	...
	1.Target: Internet Gateway and then we select  IGW_A19_P1
1. Click on "save routes" and Close

### Step 5: Create the Private Subnet
1. Click on "Subnets" from the left tab and then on the button "Create Subnet"
	1. Name tag: **PrivateSubnet1_A19_P1**
	1. VPC: select the ID for the DSTI_A19_Project1 VPC
	1. IPv4 CIDR block: **14.80.2.0/23** IP range between 14.800.2.0 and 14.80.3.255
1. Click on Create and finally on Close
...
1. Click on the left tab list on "Route Tables" and then on the button "Create route table"
	1. Name tag: **Private_Access_A19_Project1**
	1. VPC: select the ID for the DSTI_A19_Project1 VPC
	1. Click on Create and Close
...
1. In the pop-up window select to Choose an existing key pair: A19_Project1 and click on the checkbox
1. And finally click on the button "Launch Instances"
	1. Note this NAT instance ID: i-0e1c24a2937b8a993 
...
1. We go to Instances, we select the JB_A19_P1 and we note down its Public IP: 54.146.87.81
1. Similarly, we select the FI_A19_P1 and we note down its Private IP: 14.80.2.213
...
```
1. From here, we issue the following command to ssh to the FI instance
```sh
